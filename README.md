# Primeros Pasos y Eficiencia Diaria

Este curso práctico permite a profesionales de proyectos, mantenimiento y energía incorporar Copilot en su rutina diaria para optimizar tareas administrativas y operativas. A través de ejercicios aplicados, los participantes aprenderán a interactuar eficazmente con la IA mediante prompting y a utilizar Copilot en la gestión de proyectos, organización de tareas y elaboración de reportes básicos, mejorando la eficiencia y calidad de sus entregables.

## Estructura

- `CapituloXX/README.md`: guía de laboratorio por capítulo.

## Lista de laboratorios

### Capítulo 1

- [Cómo hablarle a la IA: Contexto, Tarea y Formato.](Capitulo01/README.md#cómo-hablarle-a-la-ia-contexto-tarea-y-formato)
  - Descripción: Actividad práctica guiada basada estrictamente en el contenido del módulo.
  - Duración estimada: 60 min

### Capítulo 2

- [Creación de casos de negocios, actas, cronogramas preliminares, interesados, lecciones aprendidas y redacción de correos contractuales.](Capitulo02/README.md#creación-de-casos-de-negocios-actas-cronogramas-preliminares-interesados-lecciones-aprendidas-y-redacción-de-correos-contractuales)
  - Descripción: Actividad práctica guiada basada estrictamente en el contenido del módulo.
  - Duración estimada: 180 min

### Capítulo 3

- [Uso de Copilot para priorizar listas de pendientes, estructurar pasos para una tarea técnica y extraer puntos clave de manuales extensos.](Capitulo03/README.md#uso-de-copilot-para-priorizar-listas-de-pendientes-estructurar-pasos-para-una-tarea-técnica-y-extraer-puntos-clave-de-manuales-extensos)
  - Descripción: Actividad práctica guiada basada estrictamente en el contenido del módulo.
  - Duración estimada: 90 min

### Capítulo 4

- [Creación de borradores para reportes de turno, listas de verificación de seguridad y formatos de inspección simple.](Capitulo04/README.md#1-práctica-4-creación-de-borradores-para-reportes-de-turno-listas-de-verificación-de-seguridad-y-formatos-de-inspección-simple)
  - Descripción: Actividad práctica guiada basada estrictamente en el contenido del módulo.
  - Duración estimada: 90 min

## Flujo de colaboración

- Trabajar en `changes_course`.
- Crear Pull Request hacia `main`.
- Merge por `Squash and merge`.
